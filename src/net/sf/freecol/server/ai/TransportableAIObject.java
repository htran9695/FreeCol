...
/**
 * A single item in a carrier's transport list. Any {@link Locatable} which
 * should be able to be transported by a carrier using the
 * {@link net.sf.freecol.server.ai.mission.TransportMission}, should extend this
 * class.
 *
...
public abstract class TransportableAIObject extends ValuedAIObject {
	...
	/**
	 * The priority of tools intended for a Colony with none stored at the
	 * present (and with no special needs).
	 */
	public static final int TOOLS_FOR_COLONY_PRIORITY = 10;

	/**
	 * The extra priority value added to the base value of
	 * {@link #TOOLS_FOR_COLONY_PRIORITY} for each ColonyTile needing a terrain
	 * improvement.
	 */
	...
	/**
	 * The extra priority value added to the base value of
	 * {@link #TOOLS_FOR_COLONY_PRIORITY} if a building is lacking tools. The
	 * number of tools is also added to the total amount.
	 */
	public static final int TOOLS_FOR_BUILDING = 100;

	/**
	 * The <code>AIUnit</code> which has been allocated to transport this
	 * object.
	 */
	 ...
	 *
	 * @param transport
	 *            The new carrier <code>AIUnit</code>.
	 */
	 ...
	 * TransportMission lists, and disembark from the old carrier if possible.
	 *
	 * @param aiCarrier
	 *            The new carrier <code>AIUnit</code>.
	 * @return True if the transport was changed, false if the transportable was
	 *         unable to disembark from the old carrier or unable to be added to
	 *         the new carrier transport list.
	 */
	...
	/**
	 * Get the source location for this transportable AI object. This is
	...
	/**
	 * Get the destination location for this transportable AI object. This can
	 * be the target {@link net.sf.freecol.common.model.Tile} of the transport
	 * or the target of the mission.
	 *
	...
	/**
	 * Get the path to deliver this transportable to its destination with a
	 * given carrier.
	 *
	 * @param carrier
	 *            The carrier <code>Unit</code> to use.
	 * @param dst
	 *            The destination <code>Location</code>, defaulting to the
	...
	/**
	 * Get the path to make progress with this transport, for the carrier to
	 * either collect or deliver the transportable, albeit it need only improve
	 * the current situation rather than complete the delivery to the
	 * destination.
	 *
	 * @param carrier
	 *            The carrier <code>Unit</code> to use.
	 ...
	 *
	 * @param carrier
	 *            The potential carrier <code>Unit</code>.
	 * @return True if the unit can carry this transportable.
	 */
	public abstract boolean carriableBy(Unit carrier);

	/**
	 * This transportable can move now. Useful for units that may or may not
	 * have enough moves left to join or leave a carrier.
	 *
	 ...
	 *
	 * @param direction
	 *            The <code>Direction</code> to leave in, null to leave in
	 *            place.
	 * @return True if the object successfully left the carrier.
	 */
	 ...
	 *
	 * @param carrier
	 *            The carrier <code>Unit</code> to join.
	 * @param direction
	 *            The <code>Direction</code> to move, null to join a carrier in
	 *            the same location.
	...
	protected void readAttributes(FreeColXMLReader xr) throws XMLStreamException {
		...
		transport = (xr.hasAttribute(TRANSPORT_TAG))
				? xr.makeAIObject(aiMain, TRANSPORT_TAG, AIUnit.class, (AIUnit) null, true) : null;
	}
}
