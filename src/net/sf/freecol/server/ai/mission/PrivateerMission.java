...
public class PrivateerMission extends Mission {
	...
	/**
	 * The target for this mission. - A port location to drop off plunder - A
	 * unit to attack - An unexplored tile
	 */
	...
	public int getDistanceToTarget() {
		return (target == null || target.getTile() == null || !getUnit().hasTile()) ? 1000000
	...
	public static Location extractTarget(AIUnit aiUnit, PathNode path) {
		...
		return (loc instanceof Europe) ? loc
				: (other != null && invalidUnitReason(aiUnit, other) == null) ? other
						: (settlement != null && invalidTargetReason(settlement, owner) == null) ? settlement : null;
	}
	 ...
	 *
	 * @param aiUnit
	 *            The <code>AIUnit</code> that may attack.
	 * @param defender
	 ...
	 * @param deferOK
	 *            Enable colony fallback (not implemented).
	 ...
	 *
	 * @param aiUnit
	 *            The <code>AIUnit</code> to find a path for.
	 * @param range
	 *            The maximum number of turns to seek for a target.
	 * @param deferOK
	...
	private static String invalidSettlementReason(AIUnit aiUnit, Settlement settlement) {
		return (settlement instanceof Colony) ? invalidTargetReason(settlement, aiUnit.getUnit().getOwner())
								...
								: (loc instanceof Europe) ? invalidTargetReason(loc, owner)
										: (loc instanceof Settlement)
												...
												: (loc instanceof Unit) ? invalidUnitReason(aiUnit, (Unit) loc)
														: (loc instanceof Tile)
																? ((((Tile) loc).isExploredBy(owner))
																		? "tile-is-explored" : null)
																: Mission.TARGETINVALID;
	}
