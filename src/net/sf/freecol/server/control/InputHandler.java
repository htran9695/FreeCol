...
public abstract class InputHandler extends FreeColServerHolder implements MessageHandler {
	...
	public InputHandler(final FreeColServer freeColServer) {
		...
		register("logout", (Connection connection, Element element) -> logout(connection, element));
		register("disconnect", new DisconnectHandler());
		register("chat", (Connection connection, Element element) -> new ChatMessage(getGame(), element)
	...
	private class DisconnectHandler implements NetworkRequestHandler {
		 ...
		 * 
		 * @see
		 * net.sf.freecol.common.networking.NetworkRequestHandler#handle(net.sf.
		 * freecol.common.networking.Connection, org.w3c.dom.Element)
		 */
		@Override
