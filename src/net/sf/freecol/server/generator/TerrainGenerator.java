...
public class TerrainGenerator {
	 ...
	 *
	 * Uses of the PRNG are usually logged in FreeCol, but the use is so intense
	 * here and this code is called pre-game, so we intentionally skip the
	 * logging here.
	 */
	 ...
	 * @param importGame
	 *            A <code>Game</code> to selectively import from.
	 * @param random
	 ...
	 * @param landMap
	 *            A boolean array defining where the land is.
	 * @param latitude
	 *            The tile latitude.
	 ...
	 *
	 * @param latitude
	 *            The location of the tile relative to the north/south poles and
	 *            equator: 0 is the mid-section of the map (equator) +/-90 is on
	 *            the bottom/top of the map (poles).
	...
	private TileType getRandomLandTileType(int latitude) {
		if (landTileTypes == null) {
			...
			landTileTypes = spec.getTileTypeList().stream().filter(t -> !t.isElevation() && !t.isWater())
	 ...
	 *
	 * @param latitude
	 *            The latitude of the proposed tile.
	...
	private TileType getRandomOceanTileType(int latitude) {
		if (oceanTileTypes == null) {
			oceanTileTypes = spec.getTileTypeList().stream()
					.filter(t -> t.isWater() && t.isHighSeasConnected() && !t.isDirectlyHighSeasConnected())
					.collect(Collectors.toList());
		}
	 ...
	 *
	 * For the remaining land tiles, one region per contiguous landmass is
	 * created.
	 *
	 * @param map
	 *            The <code>Map</code> to work on.
	 * @param lb
	...
	private List<ServerRegion> createLandRegions(Map map, LogBuilder lb) {
		...
		for (int c = 1; c <= continents; c++) {
			...
			int score = Math.max((int) (((float) sr.getSize() / landsize) * LAND_REGIONS_SCORE_VALUE),
	 ...
	 *
	 * @param map
	 *            A <code>Map</code> to choose from.
	...
	/**
	 * Creates mountain ranges on the given map. The number and size of mountain
	 * ranges depends on the map size.
	 *
	 * @param map
	 *            The map to use.
	 * @param lb
	...
	private List<ServerRegion> createMountains(Map map, LogBuilder lb) {
		...
		// mountain ranges
		int maximumLength = Math.max(mapOptions.getInteger(MapGeneratorOptions.MAP_WIDTH),
				mapOptions.getInteger(MapGeneratorOptions.MAP_HEIGHT)) / 10;
		int number = (int) ((1.0f - randomHillsRatio)
				* (getApproximateLandCount() / mapOptions.getInteger(MapGeneratorOptions.MOUNTAIN_NUMBER)));
		...
		for (int tries = 0; tries < 100; tries++) {
			...
			Direction direction = Direction.getRandomDirection("getLand", logger, random);
	...
	/**
	 * Creates rivers on the given map. The number of rivers depends on the map
	 * size.
	 *
	 * @param map
	 *            The <code>Map</code> to create rivers on.
	 * @param lb
	 *            A <code>LogBuilder</code> to log to.
	 * @return A list of created <code>ServerRegion</code>s.
	 */
	...
	private List<ServerRegion> createLakeRegions(Map map, LogBuilder lb) {
		...
		for (int y = 0; y < map.getHeight(); y++) {
			for (int x = 0; x < map.getWidth(); x++) {
				Tile tile;
				if (map.isValid(x, y) && !(tile = map.getTile(x, y)).isLand()
	 ...
	 *
	 * // @compat 0.10.x // Public/static to allow Map to remediate missing lake
	 * regions // end @compat 0.10.x
	 *
	 * @param map
	 *            The <code>Map</code> to add to.
	 * @param lakes
	 *            A list of lake <code>Tile</code>s.
	 ...
	 *
	 * @param t
	 *            The <code>Tile</code> to add bonuses to.
	 * @param generateBonus
	...
	private void perhapsAddBonus(Tile t, boolean generateBonus) {
		...
		final int bonusNumber = mapOptions.getInteger(MapGeneratorOptions.BONUS_NUMBER);
		if (t.isLand()) {
			if (generateBonus && randomInt(logger, "Land Resource", random, 100) < bonusNumber) {
	 ...
	 *
	 * @param tile
	 *            The <code>Tile</code> to create the resource on.
	...
	private Resource createResource(Tile tile) {
		...
		int quantity = (minValue == maxValue) ? maxValue
				: (minValue + randomInt(logger, "Rsiz", random, maxValue - minValue + 1));
	...
	// end @compat 0.10.x
	/**
	 * Sets the style of the tiles. Only relevant to water tiles for now. Public
	 * because it is used in the river generator.
	 *
	 * @param tile
	 *            The <code>Tile</code> to set the style of.
	 */
	 ...
	 *
	 * @param landMap
	 *            The <code>LandMap</code> to use as a template.
	 * @param lb
	 *            A <code>LogBuilder</code> to log to.
	...
	public Map createMap(LandMap landMap, LogBuilder lb) {
		...
		if (importTerrain) {
			...
			map.resetHighSeas(mapOptions.getInteger(MapGeneratorOptions.DISTANCE_TO_HIGH_SEA),
