...
public final class ConnectController {
	...
	private boolean unblockServer(int port) {
		...
		if (freeColServer != null && freeColServer.getServer().getPort() == port) {
			if (gui.confirm("stopServer.text", "stopServer.yes", "stopServer.no")) {
	...
	private FreeColServer startServer(boolean publicServer, boolean singlePlayer, Specification spec, int port) {
		...
		try {
			freeColServer = new FreeColServer(publicServer, singlePlayer, spec, port, null);
		} catch (IOException e) {
			freeColServer = null;
	...
	private GameState getGameState(String host, int port) {
		...
		try (Connection mc = getConnection(host, port);) {
			if (mc == null)
				return null;
			...
			if (reply == null) {
				...
				gui.showErrorMessage("server.couldNotConnect", "bad reply: " + reply.getTagName());
				return null;
			}
	...
	public boolean login(String user, String host, int port) {
		...
		String message = null;
		try {
			if (!freeColClient.askServer().connect(FreeCol.CLIENT_THREAD + user, host, port,
		...
		if (player == null) {
			...
			gui.showErrorMessage(StringTemplate.template("server.noSuchPlayer").addName("%player%", user));
			return false;
		}
	 ...
	 *
	 * @param specification
	 *            The <code>Specification</code> for the game.
	 * @param publicServer
	 *            Whether to make the server public.
	 * @param port
	 *            The port in which the server should listen for new clients.
	...
	public boolean startMultiplayerGame(Specification specification, boolean publicServer, int port) {
		...
		if (!unblockServer(port))
			return false;

		FreeColServer freeColServer = startServer(publicServer, false, specification, port);
		if (freeColServer == null)
			return false;

		freeColClient.setFreeColServer(freeColServer);
		return joinMultiplayerGame(freeColServer.getHost(), freeColServer.getPort());
	}
	 ...
	 *
	 * @param host
	 *            The name of the machine running the server.
	 * @param port
	 *            The port to use when connecting to the host.
	...
	public boolean joinMultiplayerGame(String host, int port) {
		...
		switch (state) {
		case STARTING_GAME:
			if (!login(FreeCol.getName(), host, port))
				return false;
		...
		case IN_GAME:
			...
			for (String n : names) {
				String nam = Messages
						.message(StringTemplate.template("countryName").add("%nation%", Messages.nameKey(n)));
				choices.add(new ChoiceItem<>(nam, n));
			}
			String choice = gui.getChoice(null, Messages.message("client.choicePlayer"), "cancel", choices);
			if (choice == null)
	...
	public boolean startSinglePlayerGame(Specification spec, boolean skip) {
		...
		if (!unblockServer(FreeCol.getServerPort()))
			return false;
		...
		List<FreeColModFile> mods = freeColClient.getClientOptions().getActiveMods();
		spec.loadMods(mods);
		...
		if (!login(FreeCol.getName(), freeColServer.getHost(), freeColServer.getPort()))
			return false;
		...
		if (skip) {
			...
			gui.showStartGamePanel(freeColClient.getGame(), freeColClient.getMyPlayer(), true);
		}
		return true;
	}
	...
	public boolean startSavedGame(File file, final String userMsg) {
		...
		try (FreeColXMLReader xr = fis.getFreeColXMLReader();) {
			...
			defaultSinglePlayer = xr.getAttribute(FreeColServer.SINGLE_PLAYER_TAG, false);
			defaultPublicServer = xr.getAttribute(FreeColServer.PUBLIC_SERVER_TAG, false);
		...
		Runnable loadGameJob = () -> {
			...
			try {
				...
				freeColServer = new FreeColServer(saveGame, (Specification) null, port, name);
				freeColClient.setFreeColServer(freeColServer);
			...
			if (err != null) {
				...
				if (freeColClient.isHeadless() || FreeColDebugger.getDebugRunTurns() >= 0) {
	 ...
	 *            For example: if the server kicked us out then we don't need to
	 *            confirm with a logout message.
	 */
	...
	/**
	 * Quits the current game. If a server is running it will be stopped if
	 * stopServer is <i>true</i>. The server and perhaps the clients (if a
	 * server is running through this client and stopServer is true) will be
	 * notified.
	 *
	 * @param stopServer
	 *            Indicates whether or not a server that was started through
	 *            this client should be stopped.
	 */
	...
	public List<ServerInfo> getServerList() {
		try (Connection mc = new Connection(FreeCol.META_SERVER_ADDRESS, FreeCol.META_SERVER_PORT, null,
