...
public class GoodsDetailPanel extends ColopediaGameObjectTypePanel<GoodsType> {
	...
	public void buildDetail(String id, JPanel panel) {
		...
		if (type.isFarmed()) {
			...
			panel.add(Utility.localizedLabel("colopedia.goods.improvedBy"), "newline 20, top");
				...
				for (int index = 0; index < improvements.size(); index++) {
					...
					panel.add(
							Utility.localizedLabel(StringTemplate
									.template("colopedia.goods.improvement").addName("%name%", improvements.get(index))
		...
		if (type.isBuildingMaterial()) {
			...
			if (!buildingTypes.isEmpty()) {
				panel.add(Utility.localizedLabel("colopedia.goods.buildings"), "newline 20");
				if (allTypes) {
					JButton button = getButton(PanelType.BUILDINGS, Messages.message("colopedia.goods.allBuildings"),
							null);
			...
			if (!roles.isEmpty()) {
				panel.add(Utility.localizedLabel("colopedia.goods.equipment"), "newline 20");
				int count = 0;
				for (Role role : roles) {
			...
			if (!unitTypes.isEmpty()) {
				panel.add(Utility.localizedLabel("colopedia.goods.units"), "newline 20");
				if (allTypes) {
					JButton button = getButton(PanelType.UNITS, Messages.message("colopedia.goods.allUnits"), null);
					panel.add(button, "span");
				} else {
					int count = 0;
		...
		if (type.getBreedingNumber() < FreeColObject.INFINITY) {
			panel.add(Utility.localizedLabel("colopedia.goods.breedingNumber"), "newline 20");
			panel.add(new JLabel(Integer.toString(type.getBreedingNumber())));
		}

		panel.add(Utility.localizedLabel("colopedia.goods.description"), "newline 20");
		panel.add(Utility.localizedTextArea(Messages.descriptionKey(type), 30), "span, growx");
	}
