...
public final class RiverStyleDialog extends FreeColChoiceDialog<String> {
	...
	public RiverStyleDialog(FreeColClient freeColClient, JFrame frame) {
		...
		panel.add(Utility.localizedHeader("riverStyleDialog.text", false), "span, align center");

		List<ChoiceItem<String>> c = choices();
		c.add(new ChoiceItem<>(DELETE, DELETE)
				.setIcon(new ImageIcon(ImageLibrary.getMiscImage(ImageLibrary.DELETE, 0.5f))));
		for (String key : ResourceManager.getImageKeys(PREFIX)) {
			c.add(new ChoiceItem<>(null, key).setIcon(new ImageIcon(ResourceManager.getImage(key, 0.5f))));
		}
