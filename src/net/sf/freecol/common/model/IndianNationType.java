...
public class IndianNationType extends NationType {
	 ...
	 */
	/*
	 * public boolean canSettleTile(Tile tile) { if (tile.getType().canSettle())
	 * { return canSettleRegion(tile.getRegion()); } else { return false; } }
	 */
	/**
	 * Can this Nation can settle the given Region?
	 *
	 * @param region
	 *            a <code>Region</code> value
	 * @return a <code>boolean</code> value
	 */
	/*
	 * public boolean canSettleRegion(Region region) { if (regions.isEmpty()) {
	 * return true; } else if (regions.contains(region.getId())) { return true;
	 * } else if (region.getParent() == null) { return false; } else { return
	 * canSettleRegion(region.getParent()); } }
	 */
	 ...
	 *
	 * @param unitType
	 *            The <code>UnitType</code> skill taught.
	 * @param probability
	...
	/**
	 * Generates choices for skill that could be taught from a settlement on a
	 * given Tile.
	 *
	 * @param tile
