...
public class Game extends FreeColGameObject {
	...
	private static final java.util.Map<Class<? extends FreeColObject>, Class<? extends FreeColObject>> serverClasses = new HashMap<>();
	static {
		serverClasses.put(net.sf.freecol.common.model.Building.class, net.sf.freecol.server.model.ServerBuilding.class);
		serverClasses.put(net.sf.freecol.common.model.Colony.class, net.sf.freecol.server.model.ServerColony.class);
		serverClasses.put(net.sf.freecol.common.model.ColonyTile.class,
				net.sf.freecol.server.model.ServerColonyTile.class);
		serverClasses.put(net.sf.freecol.common.model.Europe.class, net.sf.freecol.server.model.ServerEurope.class);
		serverClasses.put(net.sf.freecol.common.model.Game.class, net.sf.freecol.server.model.ServerGame.class);
		...
		serverClasses.put(net.sf.freecol.common.model.Region.class, net.sf.freecol.server.model.ServerRegion.class);
		serverClasses.put(net.sf.freecol.common.model.Player.class, net.sf.freecol.server.model.ServerPlayer.class);
		serverClasses.put(net.sf.freecol.common.model.Unit.class, net.sf.freecol.server.model.ServerUnit.class);
	};

	/**
	 * Map of class name to class for the location classes, to speed up game
	 * loading.
	 */
	private static final java.util.Map<String, Class<? extends FreeColGameObject>> locationClasses = new HashMap<>();
	static {
		locationClasses.put("Building", net.sf.freecol.common.model.Building.class);
		locationClasses.put("Colony", net.sf.freecol.common.model.Colony.class);
		locationClasses.put("ColonyTile", net.sf.freecol.common.model.ColonyTile.class);
		locationClasses.put("Europe", net.sf.freecol.common.model.Europe.class);
		locationClasses.put("HighSeas", net.sf.freecol.common.model.HighSeas.class);
		locationClasses.put("IndianSettlement", net.sf.freecol.common.model.IndianSettlement.class);
		locationClasses.put("Map", net.sf.freecol.common.model.Map.class);
		locationClasses.put("Tile", net.sf.freecol.common.model.Tile.class);
		locationClasses.put("Unit", net.sf.freecol.common.model.Unit.class);
	};
	...
	/**
	 * The current nation options. Mainly used to see if a player nation is
	 * available.
	 */
	...
	/**
	 * References to all objects created in this game. Serialization is not
	 * needed directly as these must be completely within { players,
	 * unknownEnemy, map } which are directly serialized.
	 */
	protected final HashMap<String, WeakReference<FreeColGameObject>> freeColGameObjects = new HashMap<>(10000);

	/**
	 * The combat model this game uses. At the moment, the only combat model
	 * available is the SimpleCombatModel, which strives to implement the combat
	 * model of the original game. However, it is anticipated that other, more
	 * complex combat models will be implemented in future. As soon as that
	 * happens, we will also have to make the combat model selectable.
	 */
	...
	/**
	 * A FreeColGameObjectListener to watch the objects in the game. Usually
	 * this is the AIMain instance. FIXME: is this better done with a property
	 * change listener?
	 */
	 ...
	 *
	 * @param specification
	 *            The <code>Specification</code> for this game.
	 */
	...
	/**
	 * Creates a new <code>Game</code> object from a <code>Element</code> in a
	 * DOM-parsed XML-tree.
	 *
	 * @param element
	 *            The <code>Element</code> containing the game.
	 * @param clientUserName
	 *            The name of the owner of this view of the game.
	 */
	 ...
	 *
	 * @param go
	 *            An <code>OptionGroup</code> containing the game options.
	 */
	 ...
	 *
	 * @return An <code>OptionGroup</code> containing the map generator options.
	 */
	 ...
	 *
	 * @param mgo
	 *            An <code>OptionGroup</code> containing the map generator
	 *            options.
	 */
	...
	/**
	 * Gets the <code>FreeColGameObject</code> with the specified identifier and
	 * class.
	 *
	 * @param <T>
	 *            the generic type
	...
	/**
	 * Registers a new <code>FreeColGameObject</code> with a given identifier.
	 *
	 * @param id
	 *            The object identifier.
	 * @param fcgo
	 *            The <code>FreeColGameObject</code> to add to this
	 *            <code>Game</code>.
	 * @exception IllegalArgumentException
	 *                If either the identifier or object are null.
	 */
	...
	/**
	 * Removes the <code>FreeColGameObject</code> with the specified identifier.
	 *
	 * @param id
	 *            The object identifier.
	 * @param reason
	 *            A reason to remove the object.
	 * @exception IllegalArgumentException
	...
	/**
	 * Convenience wrapper to find a location (which is an interface, precluding
	 * using the typed version of getFreeColGameObject()) by identifier.
	 *
	 ...
	 * <code>FreeColGameObject</code>.
	 *
	 * This <code>Iterator</code> should be iterated once in a while since it
	 * cleans the <code>FreeColGameObject</code> cache. Very few routines call
	 * this any more, so there is a thresholded call in removeFreeColGameObject
	 * to ensure the cache is still cleaned. Reconsider this if the situation
	 * changes.
	 *
	...
	public Iterator<FreeColGameObject> getFreeColGameObjectIterator() {
		return new Iterator<FreeColGameObject>() {
			...
			private final Iterator<Entry<String, WeakReference<FreeColGameObject>>> it = freeColGameObjects.entrySet()
					.iterator();

			/** Read ahead to this next entry. */
			private Entry<String, WeakReference<FreeColGameObject>> readAhead = null;
	...
	/**
	 * Gets the unique identifier for this game. A game UUID persists in save
	 * game files.
	 *
	 ...
	 *
	 * @param nation
	 *            The <code>Nation</code> to search for.
	 * @return The <code>Player</code> of the given nation, or null if not
	 *         found.
	 */
	 ...
	 *
	 * @param nationId
	 *            The nation identifier to search for.
	 * @return The <code>Player</code> of the given nation, or null if not
	 *         found.
	 */
	 ...
	 *
	 * @param other
	 *            An optional <code>Player</code> to omit.
	 * @return The list of live <code>Player</code>s.
	 */
	public List<Player> getLivePlayers(Player other) {
		return players.stream().filter(p -> !p.isUnknownEnemy() && !p.isDead() && p != other)
	 ...
	 *
	 * @param players
	 *            The <code>Player</code>s to exclude.
	...
	public List<Player> getLiveEuropeanPlayers(Player other) {
		return players.stream().filter(p -> !p.isUnknownEnemy() && !p.isDead() && p != other && p.isEuropean())
				.collect(Collectors.toList());
	}
	...
	public List<Player> getLiveNativePlayers(Player other) {
		return players.stream().filter(p -> !p.isUnknownEnemy() && !p.isDead() && p != other && p.isIndian())
				.collect(Collectors.toList());
	}
	 ...
	 *
	 * @param beforePlayer
	 *            The <code>Player</code> before the <code>Player</code> to be
	 *            returned.
	 * @return The <code>Player</code> after the <code>beforePlayer</code> in
	 *         the list which determines the order each player becomes the
	 *         current player.
	 * @see #getNextPlayer
	 */
	 ...
	 *
	 * @param name
	 *            The name identifying the <code>Player</code>.
	 ...
	 *
	 * @param player
	 *            The <code>Player</code> to serve as the unknown enemy.
	 */
	...
	public Player getClientPlayer() {
		return (clientUserName == null) ? null : getPlayerByName(clientUserName);
	}
	 ...
	 *            The new current <code>Player</code>.
	 */
	 ...
	 *
	 * @param newNationOptions
	 *            The new <code>NationOptions<code> value.
	 */
	...
	public Nation getVacantNation() {
		Entry<Nation, NationState> entry = find(nationOptions.getNations().entrySet(),
	 ...
	 *
	 * @param newCombatModel
	 *            The new <code>CombatModel</code> value.
	 */
	 ...
	 *
	 * @param fcgol
	 *            The new <code>FreeColGameObjectListener</code>.
	 */
	...
	/**
	 * Maintain the player containers for certain ownables. Mainly useful in the
	 * client, informing the player that it has gained or lost an ownable.
	 *
	 * @param o
	 *            The <code>Ownable</code> that may have changed.
	 * @param oldOwner
	 *            The previous (possible unchanged) owning <code>Player</code>.
	 */
	 ...
	 * @return The <code>Settlement</code> found, or <code>null</code> if there
	 *         is no known <code>Settlement</code> with the specified name (the
	 *         settlement might not be visible to a client).
	 */
	 ...
	 *
	 * @param message
	 *            The <code>ModelMessage</code> to find the object in.
	 ...
	 * @return The corresponding server class, or the original class if none
	 *         found.
	 */
	 ...
	 *            the generic type
	 * @param returnClass
	 *            The required FreeColObject class.
	 * @param server
	 *            Create a server object if possible.
