...
public class ClaimLandMessage extends DOMMessage {
	 ...
	 * @param claimant
	 *            The <code>Unit</code> or <code>Settlement</code> claiming the
	 *            tile.
	 * @param price
	 *            The price to pay for the tile, negative if stealing.
	 */
	 ...
	 * @param connection
	 *            The <code>Connection</code> the message was from.
	 *
	...
	public Element handle(FreeColServer server, Player player, Connection connection) {
		...
		if (unit != null) {
			...
			if (settlement.getOwner().isEuropean() && !settlement.getTile().isAdjacent(tile)) {
				return DOMMessage.clientError("Settlement can not claim tile: " + tileId);
			}
		} else { // ...but not both of them.
			return DOMMessage.clientError("Not a unit or settlement: " + claimantId);
		}
		...
		if (owner == null) { // unclaimed, always free
			...
			if (settlement != null && ownerSettlement != null && tile.isInUse()) {
				return DOMMessage.clientError("Can not claim tile " + tile.getId() + ": already owned.");
			}
			price = 0;
			...
			if (tile.getOwningSettlement() == null // its not "nailed down"
				...
				return DOMMessage
						.clientError("Can not claim tile " + tile.getId() + ": European owners will not sell.");
			}
		} else { // natives
			...
			case NATIVES:
				if (price >= 0) {
					if (price < value) {
						return DOMMessage.clientError("Can not claim tile " + tile.getId() + ": insufficient offer.");
					}
					if (!player.checkGold(price)) {
						return DOMMessage
								.clientError("Can not pay for tile: " + tile.getId() + ": insufficient funds.");
					}
					// Succeed, sufficient offer
				} // else succeed, stealing
				break;
			default: // Fail
				return DOMMessage.clientError("Can not claim tile " + tile.getId() + ": " + why);
			}
		}
	...
	public Element toXMLElement() {
		return createMessage(getXMLElementTagName(), "tile", tileId, "claimant", claimantId, "price", priceString);
	}
