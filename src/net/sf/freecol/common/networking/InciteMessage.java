...
public class InciteMessage extends DOMMessage {
	...
	/**
	 * Create a new <code>InciteMessage</code> with the supplied name.
	 *
	 * @param unit
	 *            The inciting <code>Unit</code>.
	 * @param direction
	 *            The <code>Direction</code> to the settlement.
	 * @param enemy
	 *            The enemy <code>Player</code>.
	 * @param gold
	 *            The amount of gold in the bribe (negative for the initial
	 *            inquiry).
	 */
	...
	/**
	 * Create a new <code>InciteMessage</code> from a supplied element.
	 *
	 * @param game
	 ...
	 * @return An element containing the result of the incite, or an error
	...
	public Element handle(FreeColServer server, Player player, Connection connection) {
		...
		MoveType type;
		ServerPlayer enemy = game.getFreeColGameObject(enemyId, ServerPlayer.class);
		if (enemy == null) {
	...
	public Element toXMLElement() {
		return createMessage(getXMLElementTagName(), "unitId", unitId, "direction", directionString, "enemyId", enemyId,
				"gold", goldString);
	}
