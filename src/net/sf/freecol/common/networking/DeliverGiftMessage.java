...
public class DeliverGiftMessage extends DOMMessage {
	 ...
	 * @param goods
	 *            The <code>Goods</code> to deliverGift.
	 */
	...
	/**
	 * Get the <code>Unit</code> which is delivering the gift. This is a helper
	 * routine to be called in-client as it blindly trusts its field.
	 *
	...
	/**
	 * Get the <code>Settlement</code> which is receiving the gift. This is a
	 * helper routine to be called in-client as it blindly trusts its field.
	 *
	 * @return The <code>Settlement</code>, or null if none.
	 */
	public Settlement getSettlement() {
		return goods.getGame().getFreeColGameObject(settlementId, Settlement.class);
	}

	/**
	 * Get the <code>Goods</code> delivered as a gift. This is a helper routine
	 * to be called in-client as it blindly trusts its field.
	 *
	 ...
	 * @return An update containing the unit and settlement, or an error
	...
	public Element handle(FreeColServer server, Player player, Connection connection) {
		...
		if (goods.getLocation() != unit) {
			return DOMMessage.clientError("Gift " + goods.getId() + " is not with unit " + unitId);
		}

		// Proceed to deliver.
		return server.getInGameController().deliverGiftToSettlement(serverPlayer, unit, settlement, goods);
	}
	...
	public Element toXMLElement() {
		Element result = createMessage(getXMLElementTagName(), "unit", unitId, "settlement", settlementId);
