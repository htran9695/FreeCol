...
public class LoadGoodsMessage extends DOMMessage {
	 ...
	 *
	 * @param loc
	 *            The <code>Location</code> of the goods.
	 * @param type
	 *            The <code>GoodsType</code> to load.
	 * @param amount
	 *            The amount of goods to load.
	 * @param carrier
	 *            The <code>Unit</code> to load the goods onto.
	 */
	...
	/**
	 * Create a new <code>LoadGoodsMessage</code> from a supplied element.
	 *
	 * @param game
	...
	public Element handle(FreeColServer server, Player player, Connection connection) {
		...
		FreeColGameObject fcgo = player.getGame().getFreeColGameObject(locationId);
		...
		// Load the goods
		return server.getInGameController().loadGoods(serverPlayer, (Location) fcgo, type, amount, carrier);
	}
	...
	public Element toXMLElement() {
		return createMessage(getXMLElementTagName(), "location", locationId, "type", goodsTypeId, "amount",
				amountString, "carrier", carrierId);
	}
