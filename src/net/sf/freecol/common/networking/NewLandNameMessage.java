...
public class NewLandNameMessage extends DOMMessage {
	...
	/**
	 * Create a new <code>NewLandNameMessage</code> with the supplied name.
	 *
	 * @param unit
	...
	/**
	 * Create a new <code>NewLandNameMessage</code> from a supplied element.
	 *
	 * @param game
	...
	public Element handle(FreeColServer server, Player player, Connection connection) {
		...
		if (tile == null) {
			...
			return DOMMessage.clientError("Unit is not in the new world: " + unitId);
		}
		...
		// Set name.
		return server.getInGameController().setNewLandName(serverPlayer, unit, newLandName);
	}
	...
	public Element toXMLElement() {
		return createMessage(getXMLElementTagName(), "unit", unitId, "newLandName", newLandName);
	}
