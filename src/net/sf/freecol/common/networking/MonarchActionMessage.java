...
public class MonarchActionMessage extends DOMMessage {
	 ...
	 *
	 * @param action
	 *            The <code>MonarchAction</code> to do.
	 * @param template
	 *            A <code>StringTemplate</code> describing the action.
	 * @param monarchKey
	 *            The resource key for the monarch image.
	 */
	public MonarchActionMessage(MonarchAction action, StringTemplate template, String monarchKey) {
	...
	/**
	 * Create a new <code>MonarchActionMessage</code> from a supplied element.
	 *
	 * @param game
	...
	public MonarchActionMessage(Game game, Element element) {
		...
		this.action = Enum.valueOf(MonarchAction.class, element.getAttribute("action"));
	...
	public Element handle(FreeColServer server, Player player, Connection connection) {
		...
		return server.getInGameController().monarchAction(serverPlayer, action, getResult());
	}
