...
public class BuyPropositionMessage extends DOMMessage {
	...
	/**
	 * Create a new <code>BuyPropositionMessage</code> from a supplied element.
	 *
	 * @param game
	...
	public Element handle(FreeColServer server, Player player, Connection connection) {
		...
		if (goods.getLocation() != settlement) {
			return DOMMessage.clientError("Goods " + goods.getId() + " are not in settlement " + settlementId);
		}

		// Proceed to price.
		return server.getInGameController().buyProposition(serverPlayer, unit, settlement, goods, getGold());
	}
