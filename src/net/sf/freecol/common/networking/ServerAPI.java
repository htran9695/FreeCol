...
public abstract class ServerAPI {
	...
	/**
	 * Loop sending requests and handling replies from the server until they
	 * reduce to null.
	 *
	 * @param request
	 *            The initial request <code>Element</code>.
	 */
	...
	/**
	 * Sends the specified message to the server and returns the reply, if it
	 * has the specified tag. Handle "error" replies if they have a messageID or
	 * when in debug mode. This routine allows code simplification in much of
	 * the following client-server communication.
	 *
	 * In following routines we follow the convention that server I/O is
	 * confined to the ask<foo>() routine, which typically returns true if the
	 * server interaction succeeded, which does *not* necessarily imply that the
	 * actual substance of the request was allowed (e.g. a move may result in
	 * the death of a unit rather than actually moving).
	 *
	 * @param message
	 *            A <code>DOMMessage</code> to send.
	 * @param tag
	 *            The expected tag
	 * @param results
	 *            A <code>Map</code> to store special attribute results in.
	 * @return The answer from the server if it has the specified tag, otherwise
	 *         <code>null</code>.
	 */
	private Element askExpecting(DOMMessage message, String tag, HashMap<String, String> results) {
		...
		if ("error".equals(reply.getTagName())) {
			...
			logger.warning("ServerAPI. " + message.getType() + " error," + " messageId: " + messageId + " message: "
					+ messageText);
		...
		String complaint = "Received reply with tag " + reply.getTagName() + " which should have been " + tag
				+ " to message " + message;
	 ...
	 * @param results
	 *            A <code>Map</code> to store special attribute results in.
	 * @return True if the server interaction succeeded and an element with the
	 *         expected tag was found in the reply, else false.
	 */
	...
	/**
	 * Register a message handler to handle messages from the server. Used when
	 * switching from pre-game to in-game.
	 *
	 * @param messageHandler
	 *            The new <code>MessageHandler</code>.
	 */
	 ...
	 * @param host
	 *            The name of the machine running the
	...
	public boolean abandonColony(Colony colony) {
		return askHandling(new AbandonColonyMessage(colony), null, null);
	}
	 ...
	 *
	 * @param action
	 *            The monarch action responded to.
	 * @param accept
	 *            Accept or reject the offer.
	 * @return True if the server interaction succeeded.
	 */
	public boolean answerMonarch(MonarchAction action, boolean accept) {
		return askHandling(new MonarchActionMessage(action, null, "").setResult(accept), null, null);
	}
	 ...
	 * @param direction
	 *            The direction to a settlement to ask.
	 * @return True if the server interaction succeeded.
	 */
	public boolean askSkill(Unit unit, Direction direction) {
		return askHandling(new AskSkillMessage(unit, direction), null, null);
	}
	...
	public boolean assignTeacher(Unit student, Unit teacher) {
		return askHandling(new AssignTeacherMessage(student, teacher), null, null);
	}
	...
	public boolean assignTradeRoute(Unit unit, TradeRoute tradeRoute) {
		return askHandling(new AssignTradeRouteMessage(unit, tradeRoute), null, null);
	}
	...
	public boolean attack(Unit unit, Direction direction) {
		return askHandling(new AttackMessage(unit, direction), null, null);
	}
	 ...
	 *
	 * @param name
	 *            The name for the colony.
	 * @param unit
	 *            The <code>Unit</code> that will build.
	 * @return True if the server interaction succeeded.
	 */
	public boolean buildColony(String name, Unit unit) {
		return askHandling(new BuildColonyMessage(name, unit), null, null);
	}
	...
	public boolean buyFromSettlement(Unit unit, Settlement settlement, Goods goods, int gold) {
		return askHandling(new BuyMessage(unit, settlement, goods, gold), null, null);
	}
	 ...
	 * @param gold
	 *            The proposed price (including query on negative).
	...
	public int buyProposition(Unit unit, Settlement settlement, Goods goods, int gold) {
		...
		if (askHandling(new BuyPropositionMessage(unit, settlement, goods, gold), null, results)) {
			try {
	...
	public boolean cashInTreasureTrain(Unit unit) {
		return askHandling(new CashInTreasureTrainMessage(unit), null, null);
	}
	...
	public boolean changeState(Unit unit, UnitState state) {
		return askHandling(new ChangeStateMessage(unit, state), null, null);
	}
	 ...
	 * @param type
	 *            The new <code>TileImprovementType</code> to work on.
	 * @return True if the server interaction succeeded.
	 */
	public boolean changeWorkImprovementType(Unit unit, TileImprovementType type) {
		return askHandling(new ChangeWorkImprovementTypeMessage(unit, type), null, null);
	}
	...
	public boolean changeWorkType(Unit unit, GoodsType workType) {
		return askHandling(new ChangeWorkTypeMessage(unit, workType), null, null);
	}
	 ...
	 * @param claimant
	 *            The <code>Unit</code> or <code>Settlement</code> that is
	 *            claiming the tile.
	 * @param price
	 *            The amount to pay.
	 * @return True if the server interaction succeeded.
	...
	public boolean claimTile(Tile tile, FreeColGameObject claimant, int price) {
		return askHandling(new ClaimLandMessage(tile, claimant, price), null, null);
	}
	 ...
	 *
	 * @param unit
	 *            The <code>Unit</code> to operate on.
	 * @return True if the server interaction succeeded.
	 */
	public boolean clearSpeciality(Unit unit) {
		return askHandling(new ClearSpecialityMessage(unit), null, null);
	}
	...
	public boolean closeTransactionSession(Unit unit, Settlement settlement) {
		return askHandling(new CloseTransactionMessage(unit, settlement), null, null);
	}
	 ...
	 *
	 * @param nation
	 *            The name for the new nation.
	 * @param country
	 *            The name for the new country.
	 * @return True if the server interaction succeeded.
	 */
	public boolean declareIndependence(String nation, String country) {
		return askHandling(new DeclareIndependenceMessage(nation, country), null, null);
	}

	/**
	 * Server query-response for the special case of deciding to explore a
	 * rumour but then declining not to investigate the strange mounds.
	 *
	 * @param unit
	...
	public boolean declineMounds(Unit unit, Direction direction) {
		return askHandling(new DeclineMoundsMessage(unit, direction), null, null);
	}
	...
	public boolean deliverGiftToSettlement(Unit unit, Settlement settlement, Goods goods) {
		return askHandling(new DeliverGiftMessage(unit, settlement, goods), null, null);
	}
	 ...
	 *
	 * @param unit
	 *            The <code>Unit</code> that demands.
	 * @param direction
	 *            The direction to demand in.
	 * @return True if the server interaction succeeded.
	 */
	public boolean demandTribute(Unit unit, Direction direction) {
		return askHandling(new DemandTributeMessage(unit, direction), null, null);
	}
	 ...
	 *            the game
	 * @param ourUnit
	 *            Our <code>Unit</code> conducting the diplomacy.
	 * @param otherColony
	 *            The other <code>Colony</code> to negotiate with.
	 * @param agreement
	 *            The <code>DiplomaticTrade</code> agreement to propose.
	...
	public DiplomaticTrade diplomacy(Game game, Unit ourUnit, Colony otherColony, DiplomaticTrade agreement) {
		Element reply = askExpecting(new DiplomacyMessage(ourUnit, otherColony, agreement), null, null);
	 ...
	 *            the game
	 * @param ourUnit
	 *            Out <code>Unit</code> conducting the diplomacy.
	 * @param otherUnit
	...
	public DiplomaticTrade diplomacy(Game game, Unit ourUnit, Unit otherUnit, DiplomaticTrade agreement) {
		Element reply = askExpecting(new DiplomacyMessage(ourUnit, otherUnit, agreement),
	 ...
	 *            the game
	 * @param ourColony
	 *            Out <code>Colony</code> conducting the diplomacy.
	 * @param otherUnit
	 *            The other <code>Unit</code> to negotiate with.
	 * @param agreement
	 *            The <code>DiplomaticTrade</code> agreement to propose.
	...
	public DiplomaticTrade diplomacy(Game game, Colony ourColony, Unit otherUnit, DiplomaticTrade agreement) {
		Element reply = askExpecting(new DiplomacyMessage(ourColony, otherUnit, agreement),
				DiplomacyMessage.getXMLElementTagName(), null);
		if (reply == null) {
			return null;
		} else if (DiplomacyMessage.getXMLElementTagName().equals(reply.getTagName())) {
	...
	public boolean disbandUnit(Unit unit) {
		return askHandling(new DisbandUnitMessage(unit), null, null);
	}
	 ...
	 *
	 * @param unit
	 *            The <code>Unit</code> that is disembarking.
	 * @return True if the server interaction succeeded.
	 */
	public boolean disembark(Unit unit) {
		return askHandling(new DisembarkMessage(unit), null, null);
	}
	 ...
	 *
	 * @param unit
	 *            The <code>Unit</code> that is boarding.
	 * @param carrier
	 *            The carrier <code>Unit</code>.
	 * @param direction
	 *            An optional direction if the unit is boarding from an adjacent
	 *            tile, or null if from the same tile.
	 * @return True if the server interaction succeeded.
	 */
	public boolean embark(Unit unit, Unit carrier, Direction direction) {
		return askHandling(new EmbarkMessage(unit, carrier, direction), null, null);
	}
	 ...
	 *
	 * @param slot
	 *            The slot from which the unit migrates, 1-3 selects a specific
	...
	public boolean emigrate(int slot) {
		return askHandling(new EmigrateUnitMessage(slot), null, null);
	}
	...
	public boolean endTurn() {
		return askHandling(new TrivialMessage("endTurn"), null, null);
	}
	...
	public boolean enterRevengeMode() {
		return askHandling(new TrivialMessage("enterRevengeMode"), null, null);
	}
	...
	public boolean equipUnitForRole(Unit unit, Role role, int roleCount) {
		return askHandling(new EquipForRoleMessage(unit, role, roleCount), null, null);
	}
	...
	public boolean firstContact(Player player, Player other, Tile tile, boolean result) {
		return askHandling(new FirstContactMessage(player, other, tile).setResult(result), null, null);
	}
	 ...
	 * @param settlement
	 *            The <code>Settlement</code> that is trading.
	 * @return The goods for sale in the settlement, or null if the server
	 *         interaction failed.
	 */
	public List<Goods> getGoodsForSaleInSettlement(Game game, Unit unit, Settlement settlement) {
		...
		Element reply = askExpecting(message, GoodsForSaleMessage.getXMLElementTagName(), null);
		return (reply == null) ? Collections.<Goods>emptyList() : new GoodsForSaleMessage(game, reply).getGoods();
	}
	...
	public boolean getNewTradeRoute() {
		return askHandling(new TrivialMessage("getNewTradeRoute"), null, null);
	}
	...
	public java.util.Map<String, String> getStatistics() {
		...
		return (askExpecting(new TrivialMessage("getStatistics"), "statistics", results) == null) ? null : results;
	}
	 ...
	 * @param direction
	 *            The direction to a settlement to speak to.
	 * @param enemy
	 *            An enemy <code>Player</code>.
	 * @param gold
	 *            The amount of bribe, negative to enquire.
	 * @return The amount of gold to bribe with if this was an inquiry, zero for
	 *         failure to incite and >0 for successful incitement, negative if
	 *         the server interaction failed.
	 */
	 ...
	 *
	 * @param unit
	 *            The <code>Unit</code> that will join.
	 * @param colony
	 *            The <code>Colony</code> to join.
	 * @return True if the server interaction succeeded.
	 */
	public boolean joinColony(Unit unit, Colony colony) {
		return askHandling(new JoinColonyMessage(colony, unit), null, null);
	}
	...
	public boolean learnSkill(Unit unit, Direction direction) {
		return askHandling(new LearnSkillMessage(unit, direction), null, null);
	}
	...
	public boolean loadGoods(Location loc, GoodsType type, int amount, Unit carrier) {
		return askHandling(new LoadGoodsMessage(loc, type, amount, carrier), null, null);
	}
	...
	public LoginMessage login(String userName, String version) {
		Element reply = askExpecting(new TrivialMessage("login", "userName", userName, "version", version), "login",
				null);
	...
	public boolean logout() {
		return sendAndWait(new TrivialMessage("logout", "reason", "User has quit the client."));
	}

	/**
	 * Server query-response for looting. Handles both an initial query and the
	 * actual looting.
	 *
	 * @param winner
	 *            The <code>Unit</code> that is looting.
	 * @param defenderId
	 *            The identifier of the defender unit (it may have sunk).
	 * @param goods
	 *            A list of <code>Goods</code>, if empty this is a query as to
	 *            what is to be looted which is filled into the list, if
	 *            non-empty, then the list of goods to loot.
	 * @return True if the server interaction succeeded.
	 */
	public boolean loot(Unit winner, String defenderId, List<Goods> goods) {
		return askHandling(new LootCargoMessage(winner, defenderId, goods), null, null);
	}
	 ...
	 *            The missionary <code>Unit</code>.
	 * @param direction
	 *            The direction to a settlement to establish with.
	 * @param denounce
	 *            True if this is a denouncement.
	 * @return True if the server interaction succeeded.
	 */
	public boolean missionary(Unit unit, Direction direction, boolean denounce) {
		return askHandling(new MissionaryMessage(unit, direction, denounce), null, null);
	}
	...
	public boolean move(Unit unit, Direction direction) {
		return askHandling(new MoveMessage(unit, direction), null, null);
	}
	...
	public boolean moveTo(Unit unit, Location destination) {
		return askHandling(new MoveToMessage(unit, destination), null, null);
	}
	...
	public boolean newLandName(Unit unit, String name) {
		return askHandling(new NewLandNameMessage(unit, name), null, null);
	}
	 ...
	 *
	 * @param region
	 *            The <code>Region</code> that is being discovered.
	 * @param tile
	 *            The <code>Tile</code> where the region is discovered.
	 * @param unit
	 *            The <code>Unit</code> discovering the region.
	 * @param name
	 *            The new region name.
	 * @return True if the server interaction succeeded.
	 */
	public boolean newRegionName(Region region, Tile tile, Unit unit, String name) {
		return askHandling(new NewRegionNameMessage(region, tile, unit, name), null, null);
	}
	 ...
	 * @param settlement
	 *            The <code>Settlement</code> that is trading.
	 * @return An array of booleans for the buy/sell/gift status, or null if the
	...
	public boolean payArrears(GoodsType type) {
		return askHandling(new PayArrearsMessage(type), null, null);
	}
	...
	public boolean payForBuilding(Colony colony) {
		return askHandling(new PayForBuildingMessage(colony), null, null);
	}
	 ...
	 *
	 * @param unit
	 *            The <code>Unit</code> to put out.
	 * @return True if the server interaction succeeded.
	 */
	public boolean putOutsideColony(Unit unit) {
		return askHandling(new PutOutsideColonyMessage(unit), null, null);
	}
	 ...
	 *
	 * @param object
	 *            A <code>FreeColGameObject</code> to rename.
	 * @param name
	 *            The name to apply.
	 * @return True if the server interaction succeeded.
	 */
	public boolean rename(FreeColGameObject object, String name) {
		return askHandling(new RenameMessage(object, name), null, null);
	}
	...
	/**
	 * Server query-response for the dialog on scouting a native settlement,
	 * *before* choosing to speak to the chief, attack, et al.
	 *
	 * @param unit
	 *            The <code>Unit</code> that is speaking.
	 * @param direction
	 *            The direction to a settlement to ask.
	 * @return The number of settlements of this tribe (which is needed in the
	 *         dialog following), or negative on error.
	 */
	 ...
	 * @param direction
	 *            The direction to a settlement to ask.
	 * @return A string describing the result, or null if the server interaction
	 *         failed.
	 */
	...
	public int sellProposition(Unit unit, Settlement settlement, Goods goods, int gold) {
		HashMap<String, String> results = loadMap("gold");
		if (askHandling(new SellPropositionMessage(unit, settlement, goods, gold), null, results)) {
			try {
				return Integer.parseInt(results.get("gold"));
			} catch (NumberFormatException e) {
			}
	...
	public boolean sellToSettlement(Unit unit, Settlement settlement, Goods goods, int gold) {
		return askHandling(new SellMessage(unit, settlement, goods, gold), null, null);
	}
	 ...
	 *
	 * @param nation
	 *            The <code>Nation</code> to whose availability is to be set.
	 * @param state
	 *            The <code>NationState</code> defining the availability.
	 * @return True if the server interaction succeeded.
	 */
	public boolean setAvailable(Nation nation, NationState state) {
		return askHandling(new TrivialMessage("setAvailable", "nation", nation.getId(), "state", state.toString()),
				null, null);
	}
	 ...
	 *            the Colony
	 * @param buildQueue
	 *            the new values for the build queue
	 * @return True if the server interaction succeeded.
	 */
	public boolean setBuildQueue(Colony colony, List<BuildableType> buildQueue) {
		return askHandling(new SetBuildQueueMessage(colony, buildQueue), null, null);
	}
	 ...
	 * @param color
	 *            The <code>Color</code> selected.
	 * @return True if the server interaction succeeded.
	 */
	public boolean setColor(Nation nation, Color color) {
		return askHandling(
				new TrivialMessage("setColor", "nation", nation.getId(), "color", Integer.toString(color.getRGB())),
				null, null);
	}
	 ...
	 *
	 * @param unit
	 *            The <code>Unit</code> whose stop is to be updated.
	 * @param index
	 *            The stop index.
	 * @return True if the query-response succeeds.
	 */
	public boolean setCurrentStop(Unit unit, int index) {
		return askHandling(new SetCurrentStopMessage(unit, index), null, null);
	}
	...
	public boolean setDestination(Unit unit, Location destination) {
		return askHandling(new SetDestinationMessage(unit, destination), null, null);
	}
	 ...
	 *
	 * @param colony
	 *            The <code>Colony</code> where the levels are set.
	 * @param data
	 *            The <code>ExportData</code> setting.
	 * @return True if the server interaction succeeded.
	 */
	public boolean setGoodsLevels(Colony colony, ExportData data) {
		return askHandling(new SetGoodsLevelsMessage(colony, data), null, null);
	}
	 ...
	 *
	 * @param nation
	 *            The <code>Nation</code> selected.
	 * @return True if the server interaction succeeded.
	 */
	public boolean setNation(Nation nation) {
		return askHandling(new TrivialMessage("setNation", "value", nation.getId()), null, null);
	}
	 ...
	 *
	 * @param nationType
	 *            The <code>NationType</code> selected.
	 * @return True if the server interaction succeeded.
	 */
	public boolean setNationType(NationType nationType) {
		return askHandling(new TrivialMessage("setNationType", "value", nationType.getId()), null, null);
	}
	 ...
	 *
	 * @param ready
	 *            The readiness state to signal.
	 * @return True if the server interaction succeeded.
	 */
	public boolean setReady(boolean ready) {
		return send(new TrivialMessage("ready", "value", Boolean.toString(ready)));
	}
	 ...
	 *
	 * @param routes
	 *            A list of trade routes to update.
	 * @return True if the server interaction succeeded.
	 */
	public boolean setTradeRoutes(List<TradeRoute> routes) {
		return askHandling(new SetTradeRoutesMessage(routes), null, null);
	}
	...
	public boolean spy(Unit unit, Direction direction) {
		return askHandling(new SpySettlementMessage(unit, direction), null, null);
	}
	...
	public boolean trainUnitInEurope(UnitType type) {
		return askHandling(new TrainUnitInEuropeMessage(type), null, null);
	}
	 ...
	 * @param carrier
	 *            The <code>Unit</code> to unload from.
	 * @return True if the query-response succeeds.
	 */
	public boolean unloadGoods(GoodsType type, int amount, Unit carrier) {
		return askHandling(new UnloadGoodsMessage(type, amount, carrier), null, null);
	}
	 ...
	 *
	 * @param gameOptions
	 *            The <code>OptionGroup</code> containing the game options.
	 * @return True if the server interaction succeeded.
	 */
	 ...
	 *
	 * @param mapOptions
	 *            The <code>OptionGroup</code> containing the map generator
	 *            options.
	 * @return True if the server interaction succeeded.
	 */
	...
	public boolean updateTradeRoute(TradeRoute route) {
		return askHandling(new UpdateTradeRouteMessage(route), null, null);
	}
	 ...
	 *
	 * @param unit
	 *            The <code>Unit</code> to change the workLocation of.
	 * @param workLocation
	 *            The <code>WorkLocation</code> to change to.
	 * @return True if the server interaction succeeded.
	 */
	public boolean work(Unit unit, WorkLocation workLocation) {
		return askHandling(new WorkMessage(unit, workLocation), null, null);
	}
}
